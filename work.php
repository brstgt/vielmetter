<?php include 'header.php'; ?>
<h2>Leistungen</h2>
<ul class="work">
    <li>
        Koordination und Durchführung von Komplettrenovierungen und Umbauten
    </li>
    <li>
        Fliesenarbeiten
    </li>
    <li>
        <a href="#maurer">Maurerarbeiten</a>
    </li>
    <li>
        Komplettbäder
    </li>
    <li>
        Balkone
    </li>
    <li>
        Grundrißänderungen
    </li>
    <li>
        Wir betreuen sowohl Industrie- als auch Privatkunden
    </li>
    <li>
        Weitere umfassende Leistung auf Anfrage
    </li>
    <li>
        Detailausführungen
    </li>
    <li>
        Komplettsanierung, Planung und Betreuung
    </li>
    <li>
        Staubfreie Sanierung
        <a name="staubfrei"></a>
    </li>
    <li>
        Trockenbau
    </li>
    <li>
        Estricharbeiten
    </li>
    <li>
        Planung und Umsetzung von Gesamtprojekte die über unserer Kernkompetenz hinaus gehen
        (Projekt Management)
    </li>
</ul>
<div class="clear"></div>

<ul class="workThumbs">
	<li>
		<img src="gfx/work/thumb1.jpg" /><div>
			<h3>Fliesenarbeiten</h3>
			Wassertropfen aus Fliesen
		</div>
	</li>
	<li>
		<img src="gfx/work/thumb2.jpg" /><div>
			<h3>Fußbodenheizung</h3>
			Im Estrich mit niedrigem Aufbau
		</div>
	</li>
	<li>
		<img src="gfx/work/thumb3.jpg" /><div>
			<h3>Grundrißänderung</h3>
			Decksprieße erhalten die Statik
		</div>
	</li>
</ul>

<div class="clear"></div>
<a name="maurer"></a>

<h2>Maurerarbeiten</h2>
<ul class="work">
    <li>
        Schaffung von Öffnungen im nicht tragenden und tragendem Mauerwerk
    </li>
    <li>
        Lieferung und Montage von Stahlträgern und Fertigteilstürzen
    </li>
    <li>
        Einschalarbeiten an Unterzügen, Betonstützen und ähnlichen Bauteilen in Verbindung mit Bewehrungs- und
        Betonarbeiten
    </li>
    <li>
        Putz-und Beiputzarbeiten
    </li>
    <li>
        Maurer- und Betonarbeiten
    </li>
    <li>
        inklusive des notwendigen Abbruchs und der fachgerechten Entsorgung von Abbruchmaterialien
    </li>
</ul>
<div class="clear"></div>

<?php include 'footer.php'; ?>
